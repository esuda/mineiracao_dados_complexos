...
# tratamento do texto: normalização, remoção de 
# pontuação e remoção de números
...
tdm <- t(as.matrix(TermDocumentMatrix(corpus)))
head(tdm[,1:10])
...
corpus_l <- Corpus(VectorSource(Dl$text))
corpus_l <- tm_map(corpus_l, content_transformer(tolower))
corpus_l <- tm_map(corpus_l, content_transformer(removePunctuation))
corpus_l <- tm_map(corpus_l, content_transformer(removeNumbers))

# nova matriz Termo-Documento 
Ml <- as.matrix(TermDocumentMatrix(corpus_l, 
                                   control = list(stopwords = T)))
colnames(Ml) <- Dl$title
