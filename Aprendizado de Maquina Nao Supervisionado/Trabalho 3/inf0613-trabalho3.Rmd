...
outliers. Após o tratamento dos NAs normalizamos as variáveis numéricas com a técnica min-max e transformamos também as variáveis catergóricas através do one-hot encoding porém não as utilizamos como feature do modelo. Por fim, após analisar a distribuição de todas as variáveis 
numéricas, optamos por remover também as variáveis que visualmente apresentava uma distribuição muito diferente da normal e com outliers.
<!-- Fim da resposta -->
...
# Experimento com valores de eps e minPts
 ...
 print(db)
# Experimento com valores de eps e minPts
 db <- dbscan :: dbscan ( base_1 , eps = 0.35, minPts =10)
 print(db)
...
K-means pois apesar do menor número de agrupamentos, foi possível interpretar leve separação, ainda que grosseira, em positivos e negativos, no caso de DBScan, praticamente todos os pontos eram agrupados em um único grupo independentemente do número de partiçoes.
